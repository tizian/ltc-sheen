...
# original fiber-like SGGX volume layer BRDF.
...
def main():
    ...
    # Precompute the LTC fit for all inputs.
    ltc_table_sheen_volume = np.load("data/ltc_table_sheen_volume.npy")
    ...
    def eval_brdf(idx):
        ...
        A = ltc_table_sheen_volume[mu_idx, alpha_idx, 0]
        B = ltc_table_sheen_volume[mu_idx, alpha_idx, 1]
        M_inv = np.array([
            [A,   0.0, B],
            [0.0, A,   0.0],
            [0.0, 0.0, 1.0]
        ...
        R = ltc_table_sheen_volume[mu_idx, alpha_idx, 2]
    ...
    for idx in range(0, mu_res*alpha_res):
        mu_idx    = idx %  mu_res
        alpha_idx = idx // mu_res
        brdf_data_ltc[mu_idx, alpha_idx, :, :] = parallel_result[idx]
    del pool

    # ------------------------------------------------------------------------------

    # Interactive plot of the two models next to each other.
    ...
    im1 = ax[1].imshow(brdf_data_ltc[0, 0, :, :],
                       cmap="turbo",
                       extent=[0, 2*np.pi, np.pi/2, 0],
                       interpolation="nearest")
    ...
    ax[1].set_title("LTC fit", size=15)

    mu_ax    = plt.axes([0.2, 0.05, 0.2, 0.05])
    mu_sl    = matplotlib.widgets.Slider(mu_ax, "theta:", 0.0, 90.0,
                                         valinit=45.0, valstep=1.0)
    alpha_ax = plt.axes([0.6, 0.05, 0.2, 0.05])
